...
public class PreMoveNodesGen {
	...
	public static final BitBoard[] CheckLegs=new BitBoard[18];//˧ݫÿٶλ׃ҰíΈքλǥƌ
	...
	//ٲʮ͵ۡП,FileNonCapMoves[y][bitWordY][index]=newY,޸ 
		...
		for (i = 0; i < 9; i ++) {
			for (j = 0; j < 10; j ++) {	
				...
				if (IndexKing != -1) {		
					...
					PawnCheck[IndexKing].assignOr(BitMask[j >= 5 ? i * 10 + j - 1 : i * 10 + j + 1]);//ݫքǰ׽
					for (l = -1; l <= 1; l += 2) {//ݫք}ҟ
